#!/usr/bin/env npx tsx
/**
 * Script to test @abbacchio/transport package from npm
 *
 * This script installs the package from npm in a temp directory and runs a test.
 * Uses "@abbacchio/transport/transports/pino" as the transport target, just like
 * an external developer would use it.
 *
 * Usage: npx tsx scripts/js/test-npm-package.ts [options]
 ...
 *   --channel <name>  Channel name(s), comma-separated (default: npm-test)
...
function parseOptions(): Options {
  const { values } = parseArgs({
    options: {
      ...
      channel: { type: 'string', short: 'C', default: 'npm-test' },
  ...
  for (let i = 0; i < count; i++) {
    for (const channel of channels) {
      ...
      const extras = {
        ...
        name: name || randomElement(namespaces),
      };

      loggers[channel][level](extras, message);
...
async function main() {
  ...
  console.log('Testing @abbacchio/transport from npm');
  console.log('='.repeat(60));
  ...
  const tempDir = mkdtempSync(join(tmpdir(), 'abbacchio-test-'));
  ...
  try {
    ...
    console.log('\n' + '='.repeat(60));
    console.log('SUCCESS: @abbacchio/transport npm package works correctly!');
    console.log('='.repeat(60));
  } finally {
    if (options.keep) {
